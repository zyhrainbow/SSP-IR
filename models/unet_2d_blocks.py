...
class UNetMidBlock2DCrossAttn(nn.Module):
    def __init__(
        ...
        attention_type="default",
        use_image_cross_attention=False,
        image_cross_attention_dim=512, #512
    ):
        super().__init__()
...
class CrossAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        if self.use_image_cross_attention:
            for resnet, attn, image_attn, pix_attn in zip(self.resnets, self.attentions, self.image_attentions, self.pixel_attentions):
                # pop res hidden states
                res_hidden_states = res_hidden_states_tuple[-1]
                res_hidden_states_tuple = res_hidden_states_tuple[:-1]
                hidden_states = torch.cat([hidden_states, res_hidden_states], dim=1)
...
class UpBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None, upsample_size=None, pixelwise_hidden_states=None,):
