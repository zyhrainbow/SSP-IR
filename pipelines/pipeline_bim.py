...
from models.model import get_unweighted_text_embeddings, get_text_index
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def _encode_prompt(
        ...
        visual_encoder_hidden_states: Optional[torch.FloatTensor] = None,
        ...
        if prompt_embeds is None:
            ...
            text_input_ids = get_text_index(self.tokenizer,prompt)
            prompt_embeds = get_unweighted_text_embeddings(self.text_encoder,text_input_ids.to(device),chunk_length=self.tokenizer.model_max_length)
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_embeds = get_unweighted_text_embeddings(self.text_encoder, uncond_input.input_ids.to(device),chunk_length=self.tokenizer.model_max_length)
        ...
        if do_classifier_free_guidance:
            ...
            if visual_encoder_hidden_states is not None:
                visual_encoder_hidden_states = torch.cat([visual_encoder_hidden_states, visual_encoder_hidden_states])
                # visual_encoder_hidden_states = [visual_encoder_hidden_states, visual_encoder_hidden_states]

        return prompt_embeds, visual_encoder_hidden_states
    ...
    def __call__(
        ...
        start_steps = 999,
        start_point = 'noise',
        visual_encoder_hidden_states=None,
        args=None
    ):
        r"""
        ...
        prompt_embeds, visual_encoder_hidden_states = self._encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
